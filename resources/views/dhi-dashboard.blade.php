@extends('layouts.app')

@section('content')
    <script>
      tailwind.config = {
        theme: {
          extend: {
            colors: {
              primary: "#2563eb",
              secondary: "#64748b",
            },
          },
        },
      };
    </script>
  </head>
  <body class="bg-gray-50">

    <!-- Main Content -->
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <!-- Stats and Quick Actions Section -->
        <div class="lg:col-span-2 space-y-8">
          <!-- Stats Grid -->
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-white p-6 rounded-lg shadow-sm">
              <h3 class="text-gray-500 text-sm font-medium">Total Users</h3>
              <p class="mt-2 text-3xl font-semibold text-primary">156</p>
            </div>
            <div class="bg-white p-6 rounded-lg shadow-sm">
              <h3 class="text-gray-500 text-sm font-medium">Active Sessions</h3>
              <p class="mt-2 text-3xl font-semibold text-green-600">23</p>
            </div>
            <div class="bg-white p-6 rounded-lg shadow-sm">
              <h3 class="text-gray-500 text-sm font-medium">Pending Requests</h3>
              <p class="mt-2 text-3xl font-semibold text-yellow-600">12</p>
            </div>
            <div class="bg-white p-6 rounded-lg shadow-sm">
              <h3 class="text-gray-500 text-sm font-medium">System Health</h3>
              <p class="mt-2 text-3xl font-semibold text-blue-600">98%</p>
            </div>
          </div>

          <!-- Quick Actions -->
          <div class="bg-white p-6 rounded-lg shadow-sm">
            <h2 class="text-lg font-medium text-gray-900 mb-4">Quick Actions</h2>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <button
                onclick="window.location.href='user-management.html'"
                class="flex flex-col items-center justify-center p-4 bg-blue-50 rounded-lg hover:bg-blue-100 transition"
              >
                <i class="fas fa-users text-primary text-xl mb-2"></i>
                <span class="text-sm font-medium text-gray-700">Users</span>
              </button>
              <button
                onclick="window.location.href='permit-management.html'"
                class="flex flex-col items-center justify-center p-4 bg-green-50 rounded-lg hover:bg-green-100 transition"
              >
                <i class="fas fa-file-alt text-green-600 text-xl mb-2"></i>
                <span class="text-sm font-medium text-gray-700">Permits</span>
              </button>
              <button
                onclick="window.location.href='system-settings.html'"
                class="flex flex-col items-center justify-center p-4 bg-purple-50 rounded-lg hover:bg-purple-100 transition"
              >
                <i class="fas fa-cog text-purple-600 text-xl mb-2"></i>
                <span class="text-sm font-medium text-gray-700">Settings</span>
              </button>
              <button
                onclick="window.location.href='analytics.html'"
                class="flex flex-col items-center justify-center p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition"
              >
                <i class="fas fa-chart-line text-gray-600 text-xl mb-2"></i>
                <span class="text-sm font-medium text-gray-700">Analytics</span>
              </button>
            </div>
          </div>

          <!-- System Overview -->
          <div class="bg-white p-6 rounded-lg shadow-sm">
            <div class="flex items-center justify-between mb-4">
              <h2 class="text-lg font-medium text-gray-900">System Overview</h2>
              <a href="system-status.html" class="text-sm text-primary hover:text-blue-700">View Details</a>
            </div>
            <div class="space-y-4">
              <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  <p class="text-sm font-medium text-gray-900">Database Status</p>
                  <p class="text-xs text-gray-500">Last backup: 2 hours ago</p>
                </div>
                <span class="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">Healthy</span>
              </div>
              <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  <p class="text-sm font-medium text-gray-900">Security Status</p>
                  <p class="text-xs text-gray-500">Last scan: 1 hour ago</p>
                </div>
                <span class="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">Secure</span>
              </div>
              <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  <p class="text-sm font-medium text-gray-900">Server Load</p>
                  <p class="text-xs text-gray-500">Current: 45%</p>
                </div>
                <span class="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">Normal</span>
              </div>
            </div>
          </div>
        </div>

        <!-- Recent Activity Section -->
        <div class="lg:col-span-1">
          <div class="bg-white p-6 rounded-lg shadow-sm">
            <h2 class="text-lg font-medium text-gray-900 mb-4">Recent Activity</h2>
            <div class="space-y-4">
              <div class="flex items-start space-x-4">
                <div class="flex-1">
                  <p class="text-sm font-medium text-gray-900">New user registered</p>
                  <p class="text-sm text-gray-500">5 minutes ago</p>
                </div>
              </div>
              <div class="flex items-start space-x-4">
                <div class="flex-1">
                  <p class="text-sm font-medium text-gray-900">System backup completed</p>
                  <p class="text-sm text-gray-500">2 hours ago</p>
                </div>
              </div>
              <div class="flex items-start space-x-4">
                <div class="flex-1">
                  <p class="text-sm font-medium text-gray-900">Security scan completed</p>
                  <p class="text-sm text-gray-500">3 hours ago</p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </main>

    <!-- Notifications Panel -->
    <div
      id="notificationsPanel"
      class="hidden fixed right-4 top-16 w-80 bg-white rounded-lg shadow-lg border border-gray-200"
    >
      <div class="p-4 border-b border-gray-200">
        <h3 class="text-lg font-medium text-gray-900">Notifications</h3>
      </div>
      <div class="divide-y divide-gray-200 max-h-96 overflow-y-auto">
        <div class="p-4 hover:bg-gray-50">
          <p class="text-sm font-medium text-gray-900">New User Registration</p>
          <p class="text-sm text-gray-500">New vendor account created</p>
          <p class="text-xs text-gray-400 mt-1">Just now</p>
        </div>
        <div class="p-4 hover:bg-gray-50">
          <p class="text-sm font-medium text-gray-900">System Alert</p>
          <p class="text-sm text-gray-500">Database backup completed</p>
          <p class="text-xs text-gray-400 mt-1">2 hours ago</p>
        </div>
        <div class="p-4 hover:bg-gray-50">
          <p class="text-sm font-medium text-gray-900">Security Update</p>
          <p class="text-sm text-gray-500">New security patch available</p>
          <p class="text-xs text-gray-400 mt-1">3 hours ago</p>
        </div>
        <div class="p-4 hover:bg-gray-50">
          <p class="text-sm font-medium text-gray-900">Performance Alert</p>
          <p class="text-sm text-gray-500">High server load detected</p>
          <p class="text-xs text-gray-400 mt-1">4 hours ago</p>
        </div>
        <div class="p-4 hover:bg-gray-50">
          <p class="text-sm font-medium text-gray-900">Maintenance Notice</p>
          <p class="text-sm text-gray-500">Scheduled maintenance tomorrow</p>
          <p class="text-xs text-gray-400 mt-1">5 hours ago</p>
        </div>
      </div>
    </div>
  </body>
</html>

@endsection
