...
<body class="bg-gray-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-semibold text-gray-900">Permit Review</h1>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">DHI Status</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">FM Status</th>
    ...
    <script>
        ...
        function loadPermits() {
            ...
            if (statusFilter !== 'all') {
                filteredPermits = filteredPermits.filter(permit => permit.fmStatus === statusFilter);
            }
            ...
            filteredPermits.forEach(permit => {
                ...
                if (permit.status === 'pending') {
                    dhiStatusBadge = '<span class="px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">Pending</span>';
                } else if (permit.status === 'approved') {
                    dhiStatusBadge = '<span class="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">Approved</span>';
                } else if (permit.status === 'rejected') {
                    dhiStatusBadge = '<span class="px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800">Rejected</span>';
                }

                // FM Status badge
                let fmStatusBadge = '';
                if (!permit.fmStatus || permit.fmStatus === 'pending') {
                    fmStatusBadge = '<span class="px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">Pending</span>';
                } else if (permit.fmStatus === 'approved') {
                    fmStatusBadge = '<span class="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">Approved</span>';
                } else if (permit.fmStatus === 'rejected') {
                    fmStatusBadge = '<span class="px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800">Rejected</span>';
                }
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${dhiStatusBadge}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${fmStatusBadge}</td>
        ...
        function viewPermit(permitNumber) {
            ...
            document.getElementById('modalContent').innerHTML = `
                <div class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-500">DHI Status</p>
                            <p class="mt-1">${dhiStatusBadge}</p>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-500">FM Status</p>
                            <p class="mt-1">${fmStatusBadge}</p>
                        </div>
                        <div>
            ...
            // Only show action buttons for normal permits
            if (permit.priority === 'normal') {
                if (!permit.fmStatus || permit.fmStatus === 'pending') {
                    modalActions.innerHTML = `
                ...
                // For urgent permits, show a message that only DHI can approve
                modalActions.innerHTML = `
                    <span class="text-sm text-gray-500">Urgent permits require DHI approval first</span>
                `;
            }
        ...
        async function updatePermitStatus(permitNumber, newStatus) {
            ...
            // Update FM status
            permit.fmStatus = newStatus;
            permit.fmReviewedBy = 'FM Staff';
            permit.fmReviewedAt = new Date().toISOString();

            // Send email to applicant
            if (newStatus === 'approved') {
                await EmailService.notifyApplicantFMApproval(permit);
            } else if (newStatus === 'rejected') {
                await EmailService.notifyApplicantFMRejection(permit);
            }

            // Send email to DHI about FM's decision
            await EmailService.notifyDHIOfFMDecision(permit);
