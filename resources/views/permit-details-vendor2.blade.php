...
<form id="permitForm" action="{{route('permit-data')}}" method="POST">
   @csrf
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-6 flex justify-between items-center">
        <div class="flex items-center">
          <a href='{{route('permit-data')}}' class="mr-4 text-gray-500 hover:text-gray-700">
    ...
    <script>
      ...
      function approvePermit(permitId, type) {
        ...
        document.getElementById('actionButtons').innerHTML = `
          <button onclick="window.location.href='{{route('permit-data')}}" class="w-full px-4 py-2 bg-primary text-white rounded-md hover:bg-blue-700 transition">
            <i class="fas fa-arrow-left mr-2"></i> Back
          </button>
        `;
      }
...
document.getElementById('printButton').addEventListener('click', function(e) {
    ...
    var vendorId = {{ $dataVendor->id_vendor }};
    var iframe = document.getElementById('pdfIframe');

    // Sembunyikan iframe terlebih dahulu
    iframe.style.display = "none";

    // Mengambil HTML konten dari controller menggunakan AJAX
    fetch(`/view/pdf_manual_vendor/${vendorId}`)
        .then(response => response.json()) // Parse response JSON
        .then(data => {
            // Menyisipkan konten HTML ke dalam iframe
            iframe.style.display = "block"; // Menampilkan iframe setelah konten dimuat
